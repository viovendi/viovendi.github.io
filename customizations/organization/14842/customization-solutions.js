...
function enabledDisabledButton(name){
 if(calcCheckboxes(name) < 1 ){
   ...
   if( $('.'+name+' .customization2_attendee_edit-action .button-error-message').length < 1 ){     
    $('.'+name+' .customization2_attendee_further-data_headline').after('<div class="button-error-message error-text error-text--multiple">Bitte wählen Sie einen Termin an dem Sie die Messe besuchen möchten aus.</div>');
   }
 }else{
